...
from rnn_sage import *#tpair
from rnn_att import RnnAtt
from static_sage import StaticSage
from static_gat import StaticGat
from rnn_gat import RnnGat
...
class Trainer:
    ...
    def load_data(self):
        ...
        graphs_path = os.path.join(self.data_path, 'graphs', 'graphs.bin')
        ...
        self.graphs, _ = dgl.load_graphs(graphs_path)
    ...
    def train(self):
        ...
        if self.config.model == 'STHG':
            ...
            self.log_file = os.path.join(self.config.log_path, self.cmd)
            self.model_file = os.path.join(
                self.config.model_save_path,
                self.cmd + '.pt'
            )
        elif self.config.model == 'rnn_gat':
            model = RnnGat(
                self.config.input_dim,
                self.config.hidden_dim,
                self.config.output_dim,
        ...
        elif self.config.model == 'rnn_att':
            model = RnnAtt(
                self.config.input_dim,
                self.config.hidden_dim,
                self.config.output_dim,
        ...
        elif self.config.model == 'static_sage':
            model = StaticSage(
                self.config.input_dim,
                self.config.hidden_dim,
                self.config.output_dim,
        ...
        elif self.config.model == 'static_gat':
            model = StaticGat(
                self.config.input_dim,
                self.config.hidden_dim,
                self.config.output_dim,
        ...
        for epoch in range(self.config.epoch):
            ...
            with tqdm(total=len(self.train_pairs), ncols=80, desc='train epoch {}'.format(epoch)) as pbar:
                while True:
                    ...
                    # model
                    # h_init = torch.zeros(self.config.hidden_dim, requires_grad=True).to(self.device)
                    outs= model(cur_data_i, cur_data_j)

                    labels = cur_labels
                    labels = torch.Tensor(labels).to(self.device)
                    # labels = (labels <= self.time_steps).float()
                    losses = []
                    # losses_items=[]
                    for i, out in enumerate(outs):
                        ...
                        ls = self.criterion(out, (labels <= t).float())
                        losses.append(ls)
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if isinstance(cur_test_data_i[0], str) and isinstance(cur_test_data_j[0], str):
                    break
                # h_init = torch.zeros(self.config.hidden_dim).to(self.device)
